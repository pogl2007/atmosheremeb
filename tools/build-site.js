...
// Промпт ИИ-консультанта. Кладём файлом, а не вписываем в код: его читают
// и Node локально, и PHP на хостинге — так факты о компании гарантированно
// одинаковы в обоих, чего не было, когда промпт дублировался руками.
fs.writeFileSync(path.join(PUB, 'data', 'ai-prompt.txt'),
...
// Без них браузер получал бы 404 на каждой странице и сыпал ошибками в консоль.
for (const [имя, пусто] of [['overrides.json', '{}'], ['blog.json', '[]']]) {
  const п = path.join(PUB, 'data', имя);
...
fs.writeFileSync(path.join(PUB, 'data', 'admin-products.json'),
